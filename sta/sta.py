...
def load_sta_triggers(filename, duration, ntrials = 9):
    try:
        trig_t = h5py.File(filename,'r')
        h5 = True
    except:
        trig_t = loadmat(filename, squeeze_me=True)
        h5 = False
    print('file format',h5)
    #trig_t = loadmat(filename, squeeze_me=True)
    if h5 == False:
        ntrig = trig_t['timeStampMatrix'].shape[0]
    else:
        ntrig = trig_t['timeStampMatrix'].shape[1]
    print('tsm',trig_t['timeStampMatrix'].shape,'ntrig',ntrig)
    assert ntrig/ntrials is not ntrig//ntrials, 'Number of stimuli is not multiple of trials'
    dt = ntrig//ntrials
    print('dt',dt)
    if h5 == True:
        trigs = trig_t['timeStampMatrix'].value.T.astype(int)[:,0]
    else:
        trigs = trig_t['timeStampMatrix']
    print('trigs',trigs.shape)
    block_starts = np.zeros(ntrials, dtype=int)
    block_stops = np.zeros(ntrials, dtype=int)
    block_lens = np.zeros(ntrials)#, dtype=int)
    for i in range(ntrials):
        if h5 == True:
            block_starts[i] = trigs[i*dt]
            block_stops[i] = trigs[(i+1)*dt-1]
#             print(trigs[i*dt:(i+1)*dt].flatten())
            block_lens[i] = np.median(np.diff(trigs[i*dt:(i+1)*dt].flatten()))
        else:
            block_starts[i] = trigs[i*dt]
            block_stops[i] = trigs[(i+1)*dt-1]
            block_lens[i] = np.median(np.diff(trigs[i*dt:(i+1)*dt])).astype(int)
#     print(block_starts, block_stops)
...
def get_loc(f, unit):
    if f['centres'].shape[0]>4:
        return f['centres'].value[unit][:2][::-1]
    else:
        return f['centres'].value[:2,unit]
...
def compute_sta(f, unit, wn, triggers, window=(-353,4237,58), radius=15, transpose=False):
    stimindex, block_starts, block_stops = triggers
    nspace = 2*radius+1
    if transpose == True:
        sc,sr = get_loc(f, unit)
    else:
        sr,sc = get_loc(f, unit)
    #print('unit location ',sc,sr)
    spikes = get_spikes(f, unit)
    N = wn.shape[1]
